...
# have pip
# put git repository of blendertools
# pip install -e /git/BlenderTools
#
...
#sys.path.append(os.path.abspath(os.getcwd())) #import files from cwd
...
class Bake26:
  ...
  _render_height = 256
  _iAASamples = 0
  _keyframeSampleRate = 0.33  # float, <1 = more samples
  _bFitModel = False
  _perspective = True # perspective / orthographic
    # changing to ortho causes the image to shrink we'd need to change orthographic bounds as well (ortho_scale)
  ...
  def __init__(self, blendFile, outputDir, settingsPath):
    ...
    msg("render count = "+str(self._dbg_renderCount))
  ...
  def setup(self):
    ...
    if len(exists) > 0:
      ...
      if self._perspective == False:
        self._dummyCamera.data.type = 'ORTHO'
        self._dummyCamera.data.ortho_scale = 1 #tis is not right
      else:
        self._dummyCamera.data.type = 'PERSP'  
  ...
  def render(self, ob):
    ...
    if ob.hide_render == False:
      ...
      self.renderSprite(ob, None, 0, 0, 0)

      # set frame first (for all objects) as it is slowest
      # get min/max kf for all nla tracks
      min_start = 1
      max_end = 0
      ...
      while sample < max_end:
        ...
        sample += self._keyframeSampleRate

        for nla in ob.animation_data.nla_tracks:
          if not "[Action Stash]" in nla.name:
            # for strip in nla.strips: TODO: is thei needed?
            self.renderAngles(ob, nla, sample)
      self.set_frame(lastframe)
      
      msg("doing gifs")
      for nla in ob.animation_data.nla_tracks:
        if not "[Action Stash]" in nla.name:
          self.exportGif(ob, nla)
  ...
  def exportGif(self, ob, nla):
    gifms =  60.0 / float(bpy.context.scene.render.fps) / 100 * (self._keyframeSampleRate)
    msg("gifms="+str(gifms))
    gifms=round(gifms, 3)
    msg("gifms="+str(gifms))
    spt = self.spriteOutputPath(ob, nla)
    if os.path.exists(spt):
      # delay in 1/100s of a second as an integral value. 
      # Using 0.02s, 0.03s, or 0.06s for the delay is as close as it can be to standard frame rates of 60, 30, or 15 fps.
      cmd = "convert -delay " + str(int(gifms)) + " -dispose Background -loop 0 "+spt+"/*.png "+spt+"/output.gif"
      #msg("running '" + cmd + "'")
      subprocess.run(cmd, shell=True)
    else:
      msg("out path '" + spt + "'did not exist")

  def renderAngles(self, ob, track, frame):
    ...
    for zenith in range(0, self._zenith_slices + 1):
      for azumith in range(0, self._azumith_slices + 1):
        ...
        self.renderSprite(ob, track, frame, azumith, zenith)
        
  def renderSprite(self, ob, track, frame, azumith, zenith):
    #bpy.context.view_layer.objects.active = ob
    #bpy.data.objects[ob.name].select_set(True)
    fpath = self.spriteFilePath(ob, track, frame, azumith, zenith)
    bpy.context.scene.render.filepath = fpath
    bpy.ops.render.render(write_still=True)
    self._dbg_renderCount += 1

  def spriteFilePath(self, ob, track, frame, azumith, zenith):

    assert(ob)

    sep = self.spritePathSeparator()
    fname = ob.name + sep
    fname += self.spriteTrackname(track) + sep
    fname += str(azumith) + sep
    fname += str(zenith) + sep
    fname += str(frame)
    #fname += ".png"
    
    path = self.spriteOutputPath(ob, track)
    path = os.path.join(path, fname)

    return path

  def spriteOutputPath(self, ob, track):
    path = os.path.abspath(os.path.normpath(self._outPath))
    path = os.path.join(path, os.path.normpath(ob.name))
    path = os.path.join(path, os.path.normpath(self.spriteTrackname(track)))
    return path

  def spritePathSeparator(self):
    return "."

  def spriteTrackname(self, track):
    return 'default' if track == None else track.name
